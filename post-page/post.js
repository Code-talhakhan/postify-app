...
userDp.textContent = user.email[0].toUpperCase();
...
save();render();title.value="";content.value="";
};
...
<div class="comments">
${p.comments.map(c=>`<div class="comment"><b>${c.email.split("@")[0]}:</b> ${c.text}</div>`).join("")}
</div>
`;
...
posts=posts.filter(x=>x.id!==p.id);save();render();document.getElementById("modal").style.display="none";
};
...
p.comments.push({email:user.email,text:input.value});
save();render();
};
